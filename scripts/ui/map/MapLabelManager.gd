...
func initialize(p_map_controller: Node2D, p_zoom_manager: ZoomLevelManager,
				p_nation_labels: Dictionary, p_province_labels: Dictionary, p_district_labels: Dictionary) -> void:
	"""
	...
	print("MapLabelManager: Initialisiert mit %d Labels (3-Layer-Hierarchie)" % label_data_list.size())
...
func _build_label_data_list() -> void:
	...
	for nation_id in nation_labels.keys():
		...
		if label:
			var data = LabelData.new(label, "nation", nation_id, 3, 22)
	...
	for province_id in province_labels.keys():
		...
		if label:
			var data = LabelData.new(label, "province", province_id, 2, 16)
			label_data_list.append(data)
	...
	for district_id in district_labels.keys():
		...
		if label:
			var data = LabelData.new(label, "district", district_id, 1, 12)
			label_data_list.append(data)
