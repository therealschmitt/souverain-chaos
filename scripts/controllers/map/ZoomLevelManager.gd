...
func _setup_thresholds() -> void:
	...
	# REGION Layer: Nur bei maximalem Zoom-out (0.25) sichtbar, danach ausblenden
	thresholds["region"] = ZoomThreshold.new(0.15, 0.25, 0.4, 0.5)
	...
	print("ZoomLevelManager: Thresholds initialisiert (4-Layer-Hierarchie)")
	print("  Region: %.2f-%.2f (fade-in), %.2f-%.2f (fade-out)" % [0.15, 0.25, 0.4, 0.5])
...
func initialize(p_region_layer: Node2D, p_nation_layer: Node2D, p_province_layer: Node2D, p_district_layer: Node2D) -> void:
	"""
	Initialisiert Manager mit Layer-Referenzen.
	@param p_region_layer: Region-Layer
	@param p_nation_layer: Nation-Layer
	@param p_province_layer: Province-Layer
	@param p_district_layer: District-Layer
	"""
	region_layer = p_region_layer
	...
	print("ZoomLevelManager: Initialisiert mit 4 Layern (Region, Nation, Province, District)")
	print("  Initial Opacities: Region=%.2f, Nation=%.2f, Province=%.2f, District=%.2f" % [
		layer_opacities["region"],
...
func _apply_layer_opacity(layer_name: String, opacity: float) -> void:
	...
	@param layer_name: Name des Layers ("region", "nation", "province", "district")
